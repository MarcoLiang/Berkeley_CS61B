...
public class Tree234 extends IntDictionary {
  ...
  private void insert(Tree234Node n, int key) {
	  if (n == null) { // add the first key
		root = new Tree234Node(null, key);
		return;
	  }
	  
	  if (n.keys == 3) {
		  if (n.parent == null) { // root is 4-node
			  root = new Tree234Node(null, n.key2);
			  root.child1 = n.left();
			  root.child1.parent = root;
			  root.child2 = n.right();
			  root.child2.parent = root;
			  if (key < n.key2) {
				  insert(root.child1, key);
				  
			  } else {
				  insert(root.child2, key);
			  }
		  } else { // this 4-node's parents is not root
			  if (n.parent.keys == 1 && n.parent.child1 == n) {
				  n.parent.add(n.key2, n);
				  if (n.key2 > key) {
					  insert(n.parent.child1, key);
				  } else {
					  insert(n.parent.child2, key);
				  }
			  }
			  
			  if (n.parent.keys == 1 && n.parent.child2 == n) {
				  n.parent.add(n.key2, n);
				  if (n.key2 > key) {
					  insert(n.parent.child2, key);
				  } else {
					  insert(n.parent.child3, key);
				  }
			  }
			  
			  if (n.parent.keys == 2 && n.parent.child1 == n) {
				  n.parent.add(n.key2, n);
				  if (n.key2 > key) {
					  insert(n.parent.child1, key);
				  } else {
			  ...
			  if (n.parent.keys == 2 && n.parent.child2 == n) {
				  n.parent.add(n.key2, n);
				  if (n.key2 > key) {
					  insert(n.parent.child2, key);
				  } else {
			  ...
			  if (n.parent.keys == 2 && n.parent.child3 == n) {
				  n.parent.add(n.key2, n);
				  if (n.key2 > key) {
					  insert(n.parent.child3, key);
				  } else {
					  insert(n.parent.child4, key);
				  }
			  }
		  }
		  return;
